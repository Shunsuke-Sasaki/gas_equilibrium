...
electronic_energy_atoms = {
    ...
    'F': -75.06 * hartree_to_kjmol,   # kJ/mol
    'O': -99.726 * hartree_to_kjmol    # kJ/mol
}
...
def calculate_standard_enthalpy(data, composition):
    try:
        E0_M = data['E0']
        EZPE_M = data['EZPE']
...
def calculate_and_write(input_file_path, output_file_path, freq_file_path):
    ...
    with open(output_file_path, 'w') as file:
        file.write("particle,initial amount,m,Rot,Freq,Symmetry,Standard Enthalpy (kJ/mol)\n")
        for line in lines:
            ...
            symmetry = data.get(particle_name, {}).get('symmetry', "")
            
            standard_enthalpy = calculate_standard_enthalpy(data.get(particle_name, {}), composition)
...
output_file_path = 'particle_data2.txt'
freq_file_path = 'all_result'  # ガウシアンの出力ファイルのパス
